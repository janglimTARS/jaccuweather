...
// Minify JS
const jsMinifyResult = await minifyJs(jsContent, {
  compress: { drop_console: false, passes: 2 },
  mangle: true,
  format: { comments: false },
});
if (jsMinifyResult.code) {
  const before = Buffer.byteLength(jsContent, 'utf8');
  const after = Buffer.byteLength(jsMinifyResult.code, 'utf8');
  console.log(`JS minified: ${(before/1024).toFixed(1)}KB → ${(after/1024).toFixed(1)}KB (${Math.round((1-after/before)*100)}% reduction)`);
  jsContent = jsMinifyResult.code;
} else {
  console.warn('JS minification produced no output, using original');
}

// Minify HTML (ignoring MathJax LaTeX fragments which contain '<' chars that trip up the parser)
const htmlMinified = await minifyHtml(htmlContent, {
  ...
  minifyCSS: true,
  minifyJS: false, // already minified separately via terser
...
{
  const before = Buffer.byteLength(htmlContent, 'utf8');
  const after = Buffer.byteLength(htmlMinified, 'utf8');
  console.log(`HTML minified: ${(before/1024).toFixed(1)}KB → ${(after/1024).toFixed(1)}KB (${Math.round((1-after/before)*100)}% reduction)`);
  htmlContent = htmlMinified;
}
